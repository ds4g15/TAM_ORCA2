!!! 20191004A - bugfixes to allow asminc
! (throughout)
!!! /20191004A
...
CONTAINS
   ...
   SUBROUTINE rst_read
      ...
      ENDIF
#endif
      ! Initialisation of SSH
      IF (lk_dynspg_flt) THEN	         ! Filtered free surface flag
         ! copied from flt_rst
         IF(lwp) WRITE(numout,*)'Reading gcx, gcxb for SSH in ASM/OBS.'
         IF (sol_oce_alloc()/=0) THEN
            CALL ctl_stop( 'STOP', 'restart : unable to allocate sol_oce arrays' )
         ENDIF
