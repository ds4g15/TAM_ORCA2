...
!!! 20191004P - passive tracer module (if active completely bipasses normal time-stepping)
      IF ((ln_swi_opatam == 5).OR.(ln_swi_opatam == 6 )) THEN
         CALL pt_init
...
!!! 20191004J - addition of adjoint time-stepping loop
      ELSEIF (ln_swi_opatam == 3) THEN
         ...
         !!! 20191004L - application of cost function from file
            !!! 20191004M - introduction of "nn_itcf000", defining cost function averaging window
            IF ( istp >=nn_itcf000 ) THEN
               vn_ad(:,:,:)         = vn_ad(:,:,:)         + ( (vmsk_i)*zvn_tlin(:,:,:) )/(nn_itend - nn_itcf000)
               tsn_ad(:,:,:,jp_tem) = tsn_ad(:,:,:,jp_tem) + ( (tmsk_i)*ztn_tlin(:,:,:) )/(nn_itend - nn_itcf000)
            END IF
            !!! /20191004M
        !!! /20191004M
