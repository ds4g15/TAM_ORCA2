MODULE nemogcm
   ...
   USE asminc
!!! 20191004V - Added ability to apply adjoint cost function to NL ocean state (from namtst_tam)
   USE wrk_nemo
   USE dotprodfld !2019-08-02 using dot product to get scalar value of cost function  
!!! /20191004V
   IMPLICIT NONE
   PRIVATE
...
!!! 20191004V - ability to apply cost function to nonlinear model
      ...
      INTEGER :: nn_itcf000 = 0      

      CALL ctl_opn( numnam, 'namelist', 'OLD', 'FORMATTED', 'SEQUENTIAL', -1, 6, .FALSE. )
      READ( numnam, namtst_tam )        ! Namelist namctl : Control prints & Benchmark              
!!! /20191004V
...
!!! 20191004V - ability to apply adjoint cost function to nonlinear model
            IF ( ln_swi_opatam == 4 ) THEN
               IF ( istp == nit000 ) THEN
                  IF (lwp) THEN
                     CALL ctl_opn( numcfnl , 'cost_fn_background.dlm','UNKNOWN','FORMATTED', &
                          &        'SEQUENTIAL', 1, numcfnl, .FALSE., 1 )
                  END IF
                  CALL wrk_alloc(jpi,jpj,jpk,zvn_trjcf)
                  ...
                  CALL lbc_lnk(zvn_trjcf(:,:,:), 'V', -1.0_wp)
                  CALL lbc_lnk(ztn_trjcf(:,:,:), 'T', 1.0_wp)
#endif
                  CALL iom_close( ncidcf )
               END IF
               cf_nltot=DOT_PRODUCT(zvn_trjcf(:,:,:),vn(:,:,:)) + DOT_PRODUCT(ztn_trjcf(:,:,:),tsn(:,:,:,jp_tem))

 
               IF (lwp) THEN
                  WRITE(numcfnl,*) cf_nltot
               END IF
